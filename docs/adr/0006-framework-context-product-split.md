...
**The CLI carries no brand.** The org-branded command became `ws`, its dot-directory
became `.workspace`, and its `<ORG>_USER` variable became `WS_USER`. The old names are not
repeated here: this file is published, and naming them would undo the rename.
